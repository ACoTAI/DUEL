...
def data_generator(batch_size=128, transforms=None, shuffle=True):
    ...
    demonum = 1
    map_path = "/home/xxx/catkin_ws/src/duel/expert_data/model_corridor_2"
    while demonum <= 100:
        ...
        while True:
            ...
            if count % 5 == 0:
                ...
                f = open(os.path.join(demo_path, 'speed' + ".txt"), 'a')
                f.write(str(number) + ' ' + str(move_cmd.get_msg().linear.x) + ' ' + str(move_cmd.get_msg().angular.z) + ' ' + str(
                    damage) + '\n')
                f.close()
                number += 1

            if time.time() - thetime >= 18.0:
                set_start()
                break
        demonum += 1
