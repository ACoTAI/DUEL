...
def calc_loss(act_gt, act_pred):
    vx_loss = np.average(np.abs(act_gt[:, 0] - act_pred[:, 0]))
    az_loss = np.average(np.abs(act_gt[:, 1] - act_pred[:, 1]))
    return np.array([vx_loss, az_loss])
...
def main():
    param_dir = "/home/xxx/catkin_ws/src/duel/params/params_bc/model_door/"
    demo_dir = "/home/xxx/catkin_ws/src/duel/expert_data/model_door/"
    feat_dim = [8, 10, 1024]
    aux_dim = 7
    encode_dim = 2
    action_dim = 2
    ...
    feat_extractor = Model(
        ...
        outputs=base_model.get_layer('activation_40').output
    )

    print "Loading data ..."
    raw = np.load(demo_dir + "model_door.npz")
    ...
    for i in xrange(episode):
        ...
        file_save.write(
            '\n' + 'Episode:' + str(i) + '  Loss:' + str(np.sum(train_loss)))
        file_save.close()

        if cur_min_loss_val > np.sum(val_loss):
            cur_min_loss_val = np.sum(val_loss)
            print("Now we save the model")
            generator.model.save_weights(param_dir + "generator_bc_model_%d.h5" % i, overwrite=True)
            with open(param_dir + "generator_bc_model_%d.json" % i, "w") as outfile:
                json.dump(generator.model.to_json(), outfile)  # turn dict to str
