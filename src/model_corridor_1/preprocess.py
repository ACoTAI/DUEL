...
def collect_pre_actions(path):
    actions_tmp = 0
    actions = 0

    file_name = path + "pre_actions.txt"
    raw = open(file_name, 'r').readlines()
    ...
    for j in xrange(0, len(raw)):
        ...
        data[0] = data[0]/0.26
        data[1] = data[1] / 1.82
        action = np.expand_dims(data[0:2], axis=0).astype(np.float32)
...
def main():
    print ("Start!")
    indicator = 0                   # Save demo, choose 0, save pre_actions, choose 1.

    if indicator == 0:
        num_patch = 80              # Change here according to the number of demo.
        demo_path = "/home/xxx/catkin_ws/src/duel/expert_data/model_corridor_1/demo_"     # Change here according to the path.

        imgs, auxs, actions = collect_demo(demo_path, num_patch)

        np.savez_compressed("/home/xxx/catkin_ws/src/duel/expert_data/model_corridor_1/model_corridor_1",
                            imgs=imgs, auxs=auxs, actions=actions)                 # Change here according to the path.

    if indicator == 1:
        demo_path = "/home/xxx/catkin_ws/src/duel/expert_data/model_corridor_1/"          # Change here according to the path.

        actions = collect_pre_actions(demo_path)

        np.savez_compressed("/home/xxx/catkin_ws/src/duel/expert_data/model_corridor_1/model_corridor_1",
                            actions=actions)                                       # Change here according to the path.
