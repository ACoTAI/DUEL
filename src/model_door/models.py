...
import math
import argparse
from keras.initializers import normal, identity, uniform  # initializations change to initializers in keras version 2.0
...
parser.add_argument("--max_step_limit", type=int, default=600)
...
parser.add_argument("--p_iter", type=int, default=500)
...
class TRPOAgent(object):
    config = dict2(paths_per_collect = args.paths_per_collect,
                   ...
                   n_iter = args.n_iter, # Number of Training
                   gamma = args.gamma,
                   lam = args.lam,
                   max_kl = args.max_kl,
    ...
    def __init__(self, env, sess, feat_dim, aux_dim, encode_dim, action_dim,
        ...
        self.env = env
        self.sess = sess
        self.buffer = ReplayBuffer(TRPOAgent.config.buffer_size)
        ...
        #                               tf.stack((tf.shape(action_dist_mu)[0], 1)))
        ...
        N = tf.shape(feats)[0]
        # compute probabilities of current actions and old actions   H in the main function
        ...
        shapes = map(var_shape, var_list)  # map var_list to anther using function var_shape
        start = 0
        tangents = []
        for shape in shapes:
            size = np.prod(shape)
        ...
        self.feat_extractor = Model(
            ...
            outputs=self.base_model.get_layer('activation_40').output
        )
    ...
    def create_generator(self, feats, auxs, encodes):
        ...
        h = LeakyReLU()(h)
        vx = Dense(1, activation='tanh', kernel_initializer='normal')(h)
        az = Dense(1, activation='tanh', kernel_initializer='normal')(h)
        actions = merge.concatenate([vx, az])
        # actions = merge([steer, accel, brake], mode='concat')
        model = Model(inputs=[feats, auxs, encodes], outputs=actions)
        return model

    def create_discriminator(self, img_dim, aux_dim, action_dim):
        imgs = Input(shape=[img_dim[0], img_dim[1], img_dim[2]])
        x = Convolution2D(32, (3, 3), strides=(2, 2))(imgs)
        x = LeakyReLU()(x)
        x = Convolution2D(64, (3, 3), strides=(2, 2))(x)
        x = LeakyReLU()(x)
        x = Convolution2D(128, (3, 3), strides=(2, 2))(x)
        x = LeakyReLU()(x)
        x = Flatten()(x)
        auxs = Input(shape=[aux_dim])
        actions = Input(shape=[action_dim])
        h = merge.concatenate([x, auxs, actions])
        h = Dense(256)(h)
        h = LeakyReLU()(h)
        h = Dense(128)(h)
        h = LeakyReLU()(h)
        p = Dense(1)(h)
        discriminate = Model(inputs=[imgs, auxs, actions], outputs=p)
        ...
        p_output = merge.add([p_n, p_d])
        # p_output = merge([p_n, p_d], mode='sum')

        model = Model(inputs=[imgs_n, auxs_n, actions_n,
                             imgs_d, auxs_d, actions_d],
                      outputs=p_output)
        rmsprop = RMSprop(lr=self.config.lr_discriminator)  # the optimize technique for discriminator
        model.compile(
    ...
    def create_posterior(self, img_dim, aux_dim, action_dim, encode_dim):
        ...
        model = Model(inputs=[imgs, auxs, actions], outputs=c)
        adam = Adam(lr=self.config.lr_posterior)  # the optimize technique for posterior
        model.compile(loss='categorical_crossentropy', optimizer=adam,
                      metrics=['accuracy'])
        return model

    def act(self, feats, auxs, encodes, logstds):
        action_dist_mu = self.sess.run(self.action_dist_mu, {self.feats: feats, self.auxs: auxs, self.encodes: encodes})

        # print "action_dist_mu :", action_dist_mu
        act = action_dist_mu + np.exp(logstds) * np.random.randn(*logstds.shape)
        print "act :", act

        act[:, 0] = np.clip(act[:, 0], -1, 1)  # limit the value of array
    ...
    def learn(self, demo):
        ...
        for i in xrange(numdetotal):
            imgs_reshaped_d.append(np.expand_dims(cv2.resize(imgs_d[i],
                (self.img_dim[1], self.img_dim[0])), axis=0))
        ...
        for i in xrange(301, config.n_iter):
            # Generating paths.
            # if i == 1:
            if i == 301:
            ...
            if imgs_n.shape[0] < config.batch_size:
                batch_size = imgs_n.shape[0] - 20
            else:
                batch_size = config.batch_size

            # batch_size = config.batch_size
            print "auxs_n shape", auxs_n.shape[0]
            ...
            for k in xrange(d_iter):
                ...
                while start_d + batch_size >= numdetotal:
                    start_d = self.demo_idx = (start_d + batch_size) % numdetotal
                while start_n + batch_size >= numnototal:
                    start_n = (start_n + batch_size) % numnototal

                print "start_d:", start_d, "start_n", start_n
                print "Discriminator step:", k, "loss:", loss

                loss_save = 'loss_d.txt'
                file_save = open(loss_save, mode='a')
                file_save.write(str(k) + ' ' + str(-loss) + '\n')
                file_save.close()
            ...
            for j in xrange(config.p_iter):
                ...
                loss_save = 'loss_p.txt'
                file_save = open(loss_save, mode='a')
                file_save.write(str(j) + ' ' + str(val_loss) + '\n')
                file_save.close()

            # Computing returns and estimating advantage function.
            path_idx = 0
            for path in paths:
                file_path = "/home/xxx/catkin_ws/src/duel/params/log/model_door/iter_%d_path_%d.txt" % (i, path_idx)
                f = open(file_path, "w")
            ...
            param_dir = "/home/xxx/catkin_ws/src/duel/params/params_0/model_door/"
