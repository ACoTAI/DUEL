...
def playGame(finetune=0):
    ...
    pre_actions_path = "/home/xxx/catkin_ws/src/duel/expert_data/model_room/pre_actions.npz"
    feat_dim = [8, 10, 1024]
    img_dim = [60, 80, 3]
    aux_dim = 7
    encode_dim = 2
    ...
    demo = np.load(demo_dir + "model_room.npz")
    ...
    try:
        if finetune:
            agent.generator.load_weights(
                param_dir + "params_0/model_room/generator_model_201.h5")
            agent.discriminator.load_weights(
                param_dir + "params_0/model_room/discriminator_model_201.h5")
            agent.baseline.model.load_weights(
                param_dir + "params_0/model_room/baseline_model_201.h5")
            agent.posterior.load_weights(
                param_dir + "params_0/model_room/posterior_model_201.h5")
            agent.posterior_target.load_weights(
                param_dir + "params_0/model_room/posterior_target_model_201.h5")
            print("load duel weight")
        else:
            agent.generator.load_weights(
                param_dir + "params_bc/model_room/generator_bc_model_37.h5")
